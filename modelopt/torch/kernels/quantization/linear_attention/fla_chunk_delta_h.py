...
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64(
    ...
    for i_t in range(NT):
        ...
        if USE_GK:
            o_k1 = tl.arange(0, 64)
            b_gk_last1 = tl.load(
                gk + (bos + last_idx) * HV * K + i_h * K + o_k1, mask=(o_k1 < K), other=0.0
            ).to(tl.float32)
            if STATE_V_FIRST:
            ...
            if K > 64:
                o_k2 = 64 + o_k1
                b_gk_last2 = tl.load(
                    gk + (bos + last_idx) * HV * K + i_h * K + o_k2, mask=(o_k2 < K), other=0.0
                ).to(tl.float32)
                if STATE_V_FIRST:
            ...
            if K > 128:
                o_k3 = 128 + o_k1
                b_gk_last3 = tl.load(
                    gk + (bos + last_idx) * HV * K + i_h * K + o_k3, mask=(o_k3 < K), other=0.0
                ).to(tl.float32)
                if STATE_V_FIRST:
            ...
            if K > 192:
                o_k4 = 192 + o_k1
                b_gk_last4 = tl.load(
                    gk + (bos + last_idx) * HV * K + i_h * K + o_k4, mask=(o_k4 < K), other=0.0
                ).to(tl.float32)
                if STATE_V_FIRST:
...
def chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64(
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        if USE_GK:
            o_k1 = tl.arange(0, 64)
            b_gk_last1 = tl.load(gk + last_idx * HV * K + o_k1, mask=(o_k1 < K), other=0.0).to(
                tl.float32
            )
        if STATE_V_FIRST:
        ...
        if K > 64:
            ...
            if USE_GK:
                b_gk_last2 = tl.load(gk + last_idx * HV * K + o_k2, mask=(o_k2 < K), other=0.0).to(
                    tl.float32
                )
            if STATE_V_FIRST:
        ...
        if K > 128:
            ...
            if USE_GK:
                b_gk_last3 = tl.load(gk + last_idx * HV * K + o_k3, mask=(o_k3 < K), other=0.0).to(
                    tl.float32
                )
            if STATE_V_FIRST:
        ...
        if K > 192:
            ...
            if USE_GK:
                b_gk_last4 = tl.load(gk + last_idx * HV * K + o_k4, mask=(o_k4 < K), other=0.0).to(
                    tl.float32
                )
            if STATE_V_FIRST:
        ...
        if STATE_V_FIRST:
            b_dh1 += tl.trans(
                tl.dot(b_q.to(b_q.dtype), b_do.to(b_q.dtype)) * scale
                - tl.dot(b_w, b_dv.to(b_w.dtype))
            )
        else:
...
def chunk_gated_delta_rule_bwd_dhu(
    ...
    if use_graph:
        ...
        dh0 = (
            get_static_buffer("dhu_dh0", tuple(h0.shape), torch.float32, h0.device)
            if h0 is not None
            else None
        )
