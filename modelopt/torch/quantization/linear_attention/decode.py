...
def _encode(value, enabled, block_v, *, state=False):
    if not enabled:
    ...
    with torch.no_grad():
        for part in value.float().split(block_v, dim=-1):
            ...
            rounded.append((part / scale).clamp(-448, 448).to(torch.float8_e4m3fn).float() * scale)
    ...
    return EncodedLinearAttentionTensor(
        _IdentityGradient.apply(value, decoded), metadata, "fp8_e4m3", block_v
    )


def _signature(config, state_qdq, block_v):
    return (
...
def _prepare_carry(q, k, v, g, beta, config, state_qdq, block_v, initial_state, carry, position):
    ...
    signature = _signature(config, state_qdq, block_v)
    ...
    if carry is None:
        ...
        carry = LinearAttentionCarry(
            _encode(initial_state, False, block_v, state=True), (), position, False, signature
        )
    ...
    if len(q) and not carry.started:
        anchor = _encode(
            carry.anchor.values, state_qdq and config.quantize_initial, block_v, state=True
        )
...
def recurrent_decode_reference(
    ...
    *,
    ...
    carry, signature = _prepare_carry(
        q, k, v, g, beta, config, state_qdq, block_v, initial_state, carry, position
    )
    if len(q) == 0:
    ...
    with torch.autocast(device_type=q.device.type, enabled=False):
        for t in range(len(q)):
            ...
            if config.mode == "token":
                anchor = _encode(working, state_qdq, block_v, state=True)
...
def recurrent_decode(
    ...
    *,
    ...
    state_format = "fp8_e4m3" if state_qdq else "identity"
    if not replay:
        anchor = EncodedLinearAttentionTensor(
            final,
