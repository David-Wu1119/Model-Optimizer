...
def _test_gdn_state_quant_helper(rank, size):
    ...
    assert all(m.gdn_state_qdq_block_v == 64 for m in gdn_modules)  # kernel's 64-column tile
    ...
    before = gdn_modules[0].in_proj.weight.detach().clone()
    optimizer.zero_grad()
    ...
    optimizer.step()
    assert not torch.equal(gdn_modules[0].in_proj.weight, before)
