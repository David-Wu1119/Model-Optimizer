...
def test_iq2_xs_cuda_extension_handles_more_blocks_than_one_grid_wave():
...
def test_iq2_xs_cuda_rejects_grid_with_wrong_shape():
    weight = torch.zeros((1, 256), device="cuda", dtype=torch.bfloat16)
    grid = iq2_xs_grid("cuda").T.contiguous()

    with pytest.raises(RuntimeError, match=r"grid must be float32 \[512, 8\]"):
        _extension().pack(weight, grid)
