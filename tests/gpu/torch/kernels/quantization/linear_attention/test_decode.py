...
@pytest.mark.parametrize("state_qdq", [False, True])
@pytest.mark.parametrize("replay", [False, True])
@pytest.mark.parametrize("read_stored", [False, True])
def test_fused_recurrence_outputs_state_and_gradients(kda, state_qdq, replay, read_stored):
    ...
    expected, carry = recurrent_decode_reference(
        q, k, v, g, beta, config=cfg, initial_state=initial, state_qdq=state_qdq, block_v=16
    )
...
def test_fused_carry_handoff_and_auxiliary_gradients(mode, interval):
    ...
    expected, expected_carry = recurrent_decode_reference(
        *args, config=reference, state_qdq=True, block_v=16, initial_state=initial
    )
    carry = None
    outputs = []
    start = 0
...
def test_wide_long_trajectory(kda, mode, horizon):
    ...
    expected, ec = recurrent_decode_reference(
        *args, config=cfg, state_qdq=True, block_v=64, initial_state=initial
    )
...
@pytest.mark.parametrize("kda", [False, True])
def test_full_key_width_and_value_block_tail(block_v, kda):
...
@pytest.mark.parametrize("kda", [False, True])
def test_packed_quantized_prefix_and_replay_composition(kda):
