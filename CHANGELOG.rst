...
- ``DistillationConfig.kd_loss_scale`` and ``DistillationConfig.skip_lm_loss`` (Megatron distillation plugin) are deprecated. ``kd_loss_scale`` is ignored with a ``DeprecationWarning``; a user-provided ``skip_lm_loss`` is overridden by the value derived from ``kd_loss_alpha`` with a ``DeprecationWarning``. The ``--no_skip_lm_loss`` and ``--kd_loss_scale`` flags in ``examples/megatron_bridge/distill.py`` are likewise deprecated and ignored.
