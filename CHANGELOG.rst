...
- Add ``four_over_six`` as a named calibration algorithm for NVFP4 Four-Over-Six, replacing the hand-written ``{method: mse, start_multiplier: 1.0, stop_multiplier: 1.5, step_size: 0.5}`` stanza it is bit-identical to. ModelOpt now rejects a config that has only one half of 4/6 -- the ``four_over_six`` block_sizes flag without a weight-scale search (``four_over_six``/``mse``/``local_hessian``), the algorithm without the flag, or the flag on anything but static NVFP4 with E4M3 scales -- each of which used to calibrate silently and produce a wrongly scaled checkpoint. ``mtq.compress`` also now refuses a 4/6 model up front instead of partway through. Writing the stanza by hand still works.
