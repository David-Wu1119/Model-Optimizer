...
- Add ``iq1_m``, ``iq2_xxs`` and ``iq2_s`` weight-only quantization, completing the GGML IQ formats at one and two bits. The same 256-value block constraint applies; these three pack with the PyTorch encoder rather than a CUDA kernel, so quantizing a large model with them is slower than with ``iq1_s`` or ``iq2_xs``.
