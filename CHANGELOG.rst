...
- Fix ``dflash_fp32_master_weights`` losing the draft's extra precision when a run resumes from an HF-format checkpoint. The draft is stored in fp32 next to a bf16 base, but ``from_pretrained(dtype="auto")`` gives every tensor one dtype, so the mantissa bits were dropped on load and a second pass over the checkpoint had to read them back. The draft is now declared to the loader as a module to keep in fp32, and ``restore_draft_precision()`` -- which no longer takes a checkpoint directory -- refuses to continue if the loader did not honour that, rather than casting a rounded copy back up and training on it.
- Fix ``dflash_fp32_master_weights`` requiring its caller to supply the bf16 autocast. The flag holds the DFlash draft in fp32 while the frozen target stays bf16, and only HF ``Trainer`` wrapped the forward that reconciles them, so every other entry point raised ``expected m1 and m2 to have the same dtype`` on the draft's first matmul: a plain ``mtsp.convert()`` followed by a forward, and ``pseudo_speculative_generate``, which AR validation calls from a Trainer callback rather than through ``forward`` -- a run with ``estimate_ar`` enabled trained normally and then died at the first ``ar_validate_steps`` boundary. The draft now enters the autocast itself, so the flag means the same thing wherever it is used, and the temporary guard that turned that crash into a named error is gone. Behaviour under the Trainer is unchanged: the model's context nests inside the Trainer's with the same device type and dtype, and it stays disabled entirely when the flag is off or the base model is already fp32.
