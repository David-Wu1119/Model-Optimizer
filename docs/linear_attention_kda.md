...
The triangular solve remains exact.
...
and `axis=(0,1)`. The policy's `state.block_v` specifies the value-column tile;
...
integration. Token-state training and decode/replay policies are separate work.
